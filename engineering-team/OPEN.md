...
| 2026-09-17 | Taggings are keyed by tag-element event id (`nostr_user_tagging.tag_event_id`, the `e` tag) while the protocol treats the `a` coordinate `39999:<author>:<slug>` as the element's stable identity and `e` as version provenance only (`tapestry/protocols/drafts/tags.md` §taggings; ADR profile/0022). Tapestry's writer now emits `a` on every tagging; ours parses only `e`, and ADR trusted-lists/0001 still records the `e`-only wire variant. Interim: `upsert_tag_element_on_db` re-points `tag_event_id` old → new on element replacement. Follow-up: read `a` in `tagging_parse`, store the coordinate, join dictionary + membership by coordinate with `e` fallback for legacy taggings, then delete the re-point. Correct ADR 0001 first. | PR #86 review thread (tagging_repo.py:54) | open |
