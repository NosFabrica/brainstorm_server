...
| `admin_whitelist.py` / `billing_admin_whitelist.py` | Who may reach `/admin/*` and `/admin/billing/*` respectively. Separate lists on purpose — billing views carry subscriber email, and whoever answers payment questions should not inherit nsec key rotation. Billing falls back to the admin list when unset, so an existing deployment keeps working; once configured it is authoritative. Both parse via `admin_whitelist.parse_pubkey_list`. |
