...
| `network_alerts_service.py` | 267 | `/networkAlerts`: resolves the observer (hex or npub) and their saved preset, builds the per-observer property keys, maps graph rows → panel payload. Neo4j plus one Postgres read for the preset. |
