...
| `billing_service.py`            | 999 | Flash entitlement on the event path: the status decision table (`decide_entitlement`, which reads the subscription's OWN `cancellationPolicy`/`dunningPolicy` rather than assuming ours — an unknown policy always holds, and the lapse sweep passes `UNKNOWN_LIFECYCLE_POLICY` because a stored row records Flash's answer and not the policy behind it), the resolution truth table (`resolve_entitlement`), and `apply_entitlement`. Also the operator writes — blocking, plan mapping, attributing or dismissing a signup that named nobody, and the three that reach Flash itself (`cancel_subscriber_subscription`, `set_subscriber_subscription_status`). Those last two write, then re-read through `apply_entitlement` unaltered, so an admin's action leaves the subscriber where the next webhook would: a pause revokes and a resume grants as a consequence of what Flash then reports, never as something the action decides. Entitlement **is** the scheduling assignment; `user_subscription` only records why. A `subscription_id` reaching `apply_entitlement` is a handle and never an authority: whatever it names entitles only the pubkey Flash says it carries, and one carrying **no** reference entitles nobody — which is what lets the checkout return pass an id straight off a redirect. `allow_unreferenced=True` is the single exception, for the admin attributing a signup that named nobody. |
| `billing_sync_service.py`       | 289 | The periodic half: replaying events we acknowledged then dropped, revoking what lapsed, re-reading Flash for what cannot be judged locally, and pruning old payloads. Split from the above because they change for different reasons; both still decide via `billing_service`, so the rules cannot drift. |
