...
| `payment_method_service.py`      | 65  | How a subscription is paid for, where that is knowable. Flash publishes no payment method per subscription (`paymentInstrumentId` is documented as withheld, and no delivery carries an acceptance method), so the only fact available is the set of methods its PLAN accepts. A plan accepting exactly ONE answers; a plan accepting several does not, and the absent key renders as nothing at all on every surface. No default and no first-of-the-list — a guessed payment method on a billing page cannot be told from a real one. |
