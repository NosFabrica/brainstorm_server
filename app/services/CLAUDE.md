...
| `designation_service.py` | 75 | Reads the Observer's latest kind-10040 from `nostr_transfer_from_relay` at enqueue time and returns the provider pubkeys in its public designation rows. They ride on the calc message as `designated_pubkeys`; the GrapeRank worker pins them at Influence 0.95 (rank 95). Best-effort: any failure yields `[]` and the run proceeds unpinned. |
