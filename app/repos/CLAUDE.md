...
| `support_repo.py` | PostgreSQL (`support_ticket`, `support_message`, `support_event`) | Ticket list statement builders (newest activity first), the unclosed-ticket count behind the cap and the per-pubkey advisory lock that makes it hold, and plain inserts. No ticket-field writes beyond insert — the service's private helpers own those |
